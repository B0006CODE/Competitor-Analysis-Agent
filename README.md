# 竞品分析 Agent v1.5.5 - Docker 部署包

## 包内容

```
competitive-analysis-agent-v1.5.5/
├── competitive-analysis-agent-v1.5.5.tar   # Docker 镜像包（docker save 导出）
├── deploy.sh                               # 一键部署脚本
...
```bash
# 1. 上传整个文件夹到服务器，进入目录
cd competitive-analysis-agent-v1.5.5
...
```bash
# 1. 加载镜像
docker load -i competitive-analysis-agent-v1.5.5.tar
...
1. 用新镜像包替换 `competitive-analysis-agent-v1.5.5.tar`
2. 重新执行 `bash deploy.sh`（自动停止旧容器并启动新容器，数据保留在 volume 中）

## 版本信息

- 版本号：v1.5.5（2026-08-21）
